...
export async function exportToPDF(transactions: any[], accountId?: string) {
  ...
  page.drawText("Dhanify – Transactions Report", {
    x: margin,
    y: y,
    size: 20,
    font: boldFont,
  ...
  for (let idx = 0; idx < transactions.length; idx++) {
    ...
    if (y < 80) {
      ...
      page.drawText(
        `Generated by Dhanify • ${new Date().toLocaleDateString("en-GB")}`,
        {
          x: width - 280,
          y: 30,
          size: 9,
      ...
      page.drawText("Dhanify – Transactions Report (continued)", {
        x: margin,
        y: y,
        size: 18,
        font: boldFont,
  ...
  const filename = accountId
    ? `dhanify_transactions_${accountId}.pdf`
    : `dhanify_transactions_${crypto.randomUUID()}.pdf`;
